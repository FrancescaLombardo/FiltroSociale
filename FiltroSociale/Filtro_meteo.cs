...
namespace FiltroSociale
{
    class Filtro_meteo
    {
        ...
        internal void LeggiParametriGlobali()
        {
            ...
            NpgsqlCommand scpostgre = new NpgsqlCommand("select distinct * from filtri.meteo_global_settings", MainProgram.con2);
            NpgsqlDataReader readerpostgre = scpostgre.ExecuteReader();

            if (!readerpostgre.HasRows)
            {
                Console.WriteLine(" Errore: non trovati dati nella tabella filtri.meteo_global_settings");
                string exname = " Errore: non trovati dati nella tabella filtri.meteo_global_settings";
            ...
            {
                string exname = " anomalia lettura dati from filtri.meteo_global_settings ";
                GestioneEccezione ecc = new GestioneEccezione();
                int livello_severita = 5;
        ...
        internal void EseguiFiltro3(double valore, DateTime data, double affiniziofiltro)
        {
            double affid = affiniziofiltro;
        ...
        internal class Filtro1
        {
            ...
            {
                NpgsqlCommand scpostgre = new NpgsqlCommand("select distinct * from filtri.meteo_filtro1_settings", MainProgram.con2);
                NpgsqlDataReader readerpostgre = scpostgre.ExecuteReader();

                if (!readerpostgre.HasRows)
                {
                    Console.WriteLine(" Errore: non trovati dati nella tabella filtri.meteo_filtro1_settings");
                    string exname = " Errore: non trovati dati nella tabella filtri.meteo_filtro1_settings";
                    GestioneEccezione ecc = new GestioneEccezione();
                    int livello_severita = 5;
                    ecc.ScriviEccezione(exname, DateTime.Now, livello_severita, MainProgram.IdProcesso);
                ...
                {
                    string exname = " anomalia lettura dati from filtri.meteo_filtro1_settings ";
                    GestioneEccezione ecc = new GestioneEccezione();
                    int livello_severita = 5;
                ...
                Console.WriteLine("Correttamente letti i parametri  filtro1 meteo ");
            }
        ...
        internal class Filtro2
        {
            ...
            {
                NpgsqlCommand scpostgre = new NpgsqlCommand("select distinct * from filtri.meteo_filtro2_settings", MainProgram.con2);
                NpgsqlDataReader readerpostgre = scpostgre.ExecuteReader();

                if (!readerpostgre.HasRows)
                {
                    Console.WriteLine(" Errore: non trovati dati nella tabella filtri.meteo_filtro2_settings");
                    string exname = " Errore: non trovati dati nella tabella filtri.meteo_filtro2_settings";
                    GestioneEccezione ecc = new GestioneEccezione();
                    int livello_severita = 5;
                    ecc.ScriviEccezione(exname, DateTime.Now, livello_severita, MainProgram.IdProcesso);
                ...
                {
                    string exname = " anomalia lettura dati from filtri.meteo_filtro2_settings ";
                    GestioneEccezione ecc = new GestioneEccezione();
                    int livello_severita = 5;
                ...
                Console.WriteLine("Correttamente letti i parametri  filtro2 meteo ");
            }
